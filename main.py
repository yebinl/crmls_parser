...
#     Used to get sellers information
def get_listing_urls3(city: str, distance: str):
    search_url = "https://matrix.crmls.org/Matrix/Search/ResidentialIncome/Detail"
    driver.get(search_url)
    time.sleep(10)
    ...
    # locate input boxes
    closed_check_box = driver.find_elements_by_xpath("//input[@data-mtx-track='Status - Closed']")
    closed_time_input_box = driver.find_elements_by_xpath("//input[@data-mtx-track='Status - Closed']/../../following-sibling::td[1]/input")
    active_check_box = driver.find_elements_by_xpath("//input[@data-mtx-track='Status - Active']")
    city_input_box = driver.find_elements_by_id("Fm13_Ctrl12_TB")
    distance_input_box = driver.find_elements_by_class_name("mapSearchDistance")

    if len(closed_check_box) < 1:
        print("Cannot locate closed check box...")
        return
    if len(closed_time_input_box) < 1:
        print("Cannot locate closed time date input box...")
        return
    if len(active_check_box) < 1:
        print("Cannot locate active check box...")
        return
    ...
    # inject filter inputs
    driver.execute_script("arguments[0].click();", closed_check_box[0])  # click closed
    driver.execute_script("arguments[0].click();", active_check_box[0])  # click active
    time.sleep(1)
    closed_time_input_box[0].send_keys(Keys.BACKSPACE, Keys.BACKSPACE, Keys.BACKSPACE, "90")
    distance_input_box[0].send_keys(Keys.BACKSPACE, Keys.BACKSPACE, distance)
    time.sleep(1)
    city_input_box[0].send_keys(city) # enter city input
    time.sleep(5)
...
def crawl_all_results(city: str):
    ...
    headers = ["Area", "Address", "Crmls ID", "Price", "Bed#", "Sqft", "Status", "Available Date", "Owner's Name", "Owner Billing Address", "Billing City & State", "Instruction", "Agent", "Agent Cell", "Agent Direct", "Agent Email", "Office Phone", "Office", "OCRealtors URL"]
        ...
        while True:
            ...
            for index in range(0, len(line_items)):
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                owner_name = ""
                            try:
                                owner_billing_address = driver.find_element_by_xpath("//td[contains(text(), 'Billing Address')]/following-sibling::td[1]").text
                            except:
                                owner_billing_address = ""
                            try:
                                owner_billing_city = driver.find_element_by_xpath("//td[contains(text(), 'Billing City')]/following-sibling::td[1]").text
                            except:
                                owner_billing_city = ""
                            driver.back()
                            time.sleep(5)
                    ...
                    line_content = [area, address, mls_listing_id, list_price, bedroom_numbers, sqft_number, active_status, available_date, owner_name, owner_billing_address, owner_billing_city, instruction_content, leasing_agent, agent_cell, agent_direct, agent_email, office_phone, leasing_office, listing_link]
...
def crawl_sellers_results(city: str):
    # view results
    result_button = driver.find_elements_by_id("m_ucResultsPageTabs_m_lbResultsTab")
    if len(result_button) < 1:
        print("Cannot locate result button...")
    ...
    headers = ["Area", "Address", "Crmls ID", "Price", "DOM/CDOM", "Sqft", "Status", "Available Date", "Owner's Name", "Owner Billing Address", "Billing City & State", "Instruction", "Agent", "Agent Cell", "Agent Direct", "Agent Email", "Office Phone", "Office", "OCRealtors URL"]
    with open(f"Output/{city}_result.csv", 'w', newline='', encoding="utf-8") as file:
        csv_writer = csv.writer(file)
        csv_writer.writerow(headers)
        ...
        while True:
            ...
            for index in range(0, len(line_items)):
                line_items = driver.find_elements_by_id("wrapperTable")
                listing_a_tag = line_items[index].find_elements_by_xpath(".//td[@class='d7m8']//a")
                if len(listing_a_tag) < 1:
                    print("Cannot locate a tag in result page...")
                    continue
                driver.execute_script("arguments[0].click();", listing_a_tag[0]) # click the listing link
                ...
                try:
                    ...
                    area = driver.find_element_by_xpath("//span[contains(text(), 'AREA:')]/following-sibling::span[2]").text
                    list_price = driver.find_element_by_xpath("//span[contains(text(), 'LIST PRICE')]/following-sibling::span[2]").text
                    bedroom_numbers = driver.find_element_by_xpath("//span[contains(text(), 'DOM / CDOM')]/following-sibling::span[2]").text
                    sqft_number = driver.find_element_by_xpath("//span[contains(text(), 'SQFT(src)')]/following-sibling::span[2]").text
                    active_status = driver.find_element_by_xpath("//span[contains(text(), 'STATUS')]/following-sibling::span[2]").text
                    if active_status == "Coming Soon":
                        available_date = driver.find_element_by_xpath("//span[contains(text(), 'STATUS')]/following-sibling::span[5]").text.replace(' ', '').replace('StartShowing:', '')
...
def crawl_ocrealtor(city: str, bedrooms="") -> None:
    try:
        ...
        # get_listing_urls2(city, 20, bedrooms)
        crawl_all_results(city+"_"+datetime.now().strftime('%Y-%m-%d,%H:%M:%S'))
...
def crawl_sellers(city: str) -> None:
    try:
        # new session in a new window for each account because of
        # selenium.common.exceptions.StaleElementReferenceException
        driver.maximize_window()
        ...
        get_listing_urls3(city, 20)
        crawl_sellers_results("Sellers_"+city+"_"+datetime.now().strftime('%Y-%m-%d,%H:%M:%S'))
...
if __name__ == '__main__':
    # crawl_ocrealtor("rialto, fontana, jurupa valley, rancho cucamonga, upland, san bernardino", "3+")
    # crawl_ocrealtor("eastvale, chino, chino hills", "3+")
    # crawl_ocrealtor("Moreno Valley, Perris, Menifee", "3+")
    crawl_sellers("Riverside")
